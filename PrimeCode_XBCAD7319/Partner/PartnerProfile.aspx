<%@ Page Title="" Language="C#" MasterPageFile="~/Partner/PartnerMaster.Master" AutoEventWireup="true" CodeBehind="PartnerProfile.aspx.cs" Inherits="PrimeCode_XBCAD7319.Partner.PartnerProfile" %>
            ...
            <ItemTemplate>
                <div class="relative flex flex-col lg:flex-row bg-[#035772] p-6 min-h-screen">
                    ...
                    <div class="flex flex-col items-center bg-[#035772] p-6 rounded-lg shadow-lg w-full lg:w-1/4">
                        <img class="rounded-full mb-4 border-4 border-white" 
                             src='<%# Eval("ProfileImage") != DBNull.Value && !string.IsNullOrEmpty(Eval("ProfileImage").ToString()) ? ResolveUrl(Eval("ProfileImage").ToString()) : ResolveUrl("~/public/images/No_image.jpg") %>' 
                        ...
                        <asp:LinkButton ID="btnUploadImage" runat="server" Text="Upload Image" CssClass="bg-[#A5C8D4] text-[#035772] font-semibold px-4 py-2 rounded-md hover:bg-[#86b1c3]" CommandName="UploadImage" CommandArgument='<%# Eval("PartnerId") %>'></asp:LinkButton>
                    </div>

                    <!-- Right Side: Partner Details and Mass Upload -->
                    <div class="flex flex-col items-center bg-white rounded-lg p-4 shadow-lg w-full lg:w-1/3 lg:ml-8 mt-8 lg:mt-0">

                        <h3 class="text-xl font-medium text-primary mb-4">Partner Details</h3>
                        ...
                        <!-- Mass File Upload Section -->
                        <div class="w-full bg-white rounded-lg p-4 shadow-lg">
                            <h3 class="text-xl font-medium text-primary mb-4">Upload Documents</h3>
                           <asp:FileUpload ID="fuMassUpload" runat="server" CssClass="w-full mb-4" Multiple="true" />
                            <asp:Button ID="btnUploadMassFiles" runat="server" CommandName="UploadMassFiles" CommandArgument='<%# Eval("PartnerId") %>' Text="Upload Mass Files" CssClass="bg-[#035772] text-white font-semibold px-4 py-2 rounded-md  hover:bg-[#A5C8D4]" />
                        </div>

                        <div class="mt-6 flex space-x-4">
                            <asp:Button ID="btnSaveChanges" runat="server" Text="Save Changes" CssClass="bg-[#035772] text-white font-semibold px-8 py-4 rounded-md hover:bg-[#A5C8D4]" CommandName="UpdateProfile" CommandArgument='<%# Eval("PartnerId") %>' OnClick="btnSaveChanges_Click" />
                        </div>
                    </div>
                </div>
            </ItemTemplate>
